...
Before a release is eligible, this repository must execute `./scripts/audit-kernel-alignment.sh` successfully in CI. The audit verifies this immutable adoption statement, confirms the repository retains its local alignment record, and confirms that at least one workflow invokes the audit. A failed or unavailable audit is not a pass and must block release promotion.
