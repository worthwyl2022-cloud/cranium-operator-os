...
failures=0
fail() {
  printf '%s\n' "KERNEL_ALIGNMENT_FAIL: $1" >&2
  failures=$((failures + 1))
}
require_file() {
  test -s "$1" || fail "required file is missing or empty: $1"
}
require_text() {
  grep -F -- "$2" "$1" >/dev/null 2>&1 || fail "required statement is absent from $1: $2"
}
...
require_text "$contract" 'Supporting repositories may submit requests, consume verified receipts, or present bounded evidence; they must not grant authority, mutate canonical state, issue canonical receipts, or define competing semantics.'
require_text "$contract" 'Local state, browser state, fixtures, generated reports, screenshots, model output, simulations, and historical material are non-canonical.'
require_text "$contract" 'Before a release is eligible, this repository must execute `./scripts/audit-kernel-alignment.sh` successfully in CI.'
...
if [ "$failures" -ne 0 ]; then
  printf '%s\n' "KERNEL_ALIGNMENT_SUMMARY: FAIL failures=$failures" >&2
  exit 1
fi
